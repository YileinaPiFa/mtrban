# MTR 圈黑名单查询网站

- 一个基于 Web 的 MTR 圈黑名单查询网站，用于查询已收录的黑名单信息，提供简洁、高效的在线查询体验
- 已**内置**API，请勿修改，会导致**无法正常使用所有功能**
- **为保护隐私，暂不支持模糊搜索|数据均经过打码处理，不展示完整账号数据**
- **展示数据仅供圈内参考，不作为实际拉黑批判**
- **请勿以本项目内容作为批判他人的依据，请勿歧视辱骂他人**

[![Demo](https://img.shields.io/badge/Demo-mtr.ylnpf.cn-2ea44f?logo=googlechrome&logoColor=white)](https://mtr.ylnpf.cn)
[![GitHub release](https://img.shields.io/github/v/release/YileinaPiFa/mtrban)](../../releases/latest)
[![GitHub downloads](https://img.shields.io/github/downloads/YileinaPiFa/mtrban/total)](../../releases)
[![License](https://img.shields.io/github/license/YileinaPiFa/mtrban)](./LICENSE)
[![GitHub stars](https://img.shields.io/github/stars/YileinaPiFa/mtrban)](../../stargazers)
[![GitHub forks](https://img.shields.io/github/forks/YileinaPiFa/mtrban)](../../network/members)
[![GitHub issues](https://img.shields.io/github/issues/YileinaPiFa/mtrban)](../../issues)
[![GitHub last commit](https://img.shields.io/github/last-commit/YileinaPiFa/mtrban)](../../commits)

## 预览

![MTR 圈黑名单查询网站](https://pan.ylnpf.cn/f/aqCR/41502C9165525A7D9850F66A9FE03CDD.png)

## 在线体验

演示地址：https://mtr.ylnpf.cn

## 功能

- 支持 MTR 圈黑名单信息查询
- 简洁直观的用户界面
- 响应式设计，兼容 PC 与移动设备
- 查询结果快速展示
- 支持自行部署与二次开发
- 注意！本项目由AI辅助开发！

## 快速开始

### 克隆仓库

```bash
git clone https://github.com/YileinaPiFa/mtrban.git
```

### 进入项目目录

```bash
cd SearchForBanWeb
```

### 安装依赖

```bash
npm install
```

### 启动开发环境

```bash
npm run dev
```

默认访问：

```
http://localhost:5173
```

### 构建生产版本

```bash
npm run build
```

### 本地预览

```bash
npm run preview
```

## 项目结构

```text
SearchForBanWeb/
├── public/              # 静态资源
├── src/                 # 项目源码
├── package.json         # 项目配置
├── vite.config.*        # Vite 配置
└── ...
```

## 技术栈

- Vue
- Vite
- JavaScript / TypeScript
- Node.js

## 贡献

欢迎提交 Issue 与 Pull Request，共同完善本项目。

## 免责声明

- 本项目仅提供公开收录信息的查询服务，仅供学习、研究与技术交流使用。
- **MTR** 在本项目中特指 **Minecraft Transit Railway** 模组交流圈，并非香港铁路（MTR Corporation）。
- 查询结果仅供参考，不代表开发者对数据的真实性、完整性或时效性作出任何保证。
- 本项目不会主动收集用户隐私信息。
- 请勿将本项目用于任何违法违规用途，由使用本项目产生的一切后果均由使用者自行承担。

## License

本项目采用 [GPL-3.0 license](./LICENSE)。

---

> 本项目/文档部分内容由AI辅助整理开发
